    ...
    <div className="space-y-6">
                ...
                <div className="p-4 space-y-2">
                  {items.map((item, j) => (
                    <div key={j} className="text-xs">
                      <code className="block bg-gray-900 text-green-400 px-3 py-1.5 rounded-lg font-mono text-[11px] leading-relaxed">
                        $ {item.cmd}
                      </code>
                      <p className="text-gray-500 mt-0.5 px-1">{item.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <FadeIn delay={400}>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-50">
              <h3 className="font-semibold text-gray-800 text-sm">🌐 HTTP Status Codes</h3>
            </div>
            <div className="p-4">
              <div className="grid grid-cols-2 gap-1.5">
                {quickReferenceStatusCodes.map((s) => (
                  <div key={s.code} className="flex items-center gap-2 p-1.5 rounded hover:bg-gray-50 text-xs">
                    <span className={`font-mono font-bold px-1.5 py-0.5 rounded text-[10px] ${
                      s.code < 300 ? 'bg-emerald-100 text-emerald-700' :
                      s.code < 400 ? 'bg-blue-100 text-blue-700' :
                      s.code < 500 ? 'bg-amber-100 text-amber-700' :
        ...
        <FadeIn delay={500}>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-50">
              <h3 className="font-semibold text-gray-800 text-sm">🔌 Critical Ports</h3>
            </div>
            <div className="p-4">
              <div className="grid grid-cols-2 gap-1.5">
                {quickReferencePorts.map((p) => (
                  <div key={p.port} className="flex items-center gap-2 p-1.5 rounded hover:bg-gray-50 text-xs">
                    <span className="font-mono font-bold text-indigo-600 w-10">{p.port}</span>
                    <span className="text-gray-700">{p.service}</span>
                    <span className="text-gray-400 text-[10px]">{p.protocol}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <FadeIn delay={600}>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-50">
            <h3 className="font-semibold text-gray-800 text-sm">📡 OSI Model — 7 Layers</h3>
          </div>
          <div className="p-4">
            <div className="grid sm:grid-cols-7 gap-2">
              {[
                ...
                <div key={l.layer} className="bg-gradient-to-b from-gray-50 to-gray-100 rounded-lg p-3 text-center border border-gray-200">
                  <div className="text-lg font-bold text-indigo-600">L{l.layer}</div>
                  <div className="text-xs font-semibold text-gray-800 mt-0.5">{l.name}</div>
                  <div className="text-[10px] text-gray-500 mt-1">{l.example}</div>
                  <div className="text-[10px] text-red-500 mt-0.5">⚠ {l.security}</div>
                </div>
              ))}
            </div>
          </div>
